...
app = FastAPI(
    title="Rule 101 — CONCATENATE Requires MODE",
    version="2.0"
)
...
class Finding(BaseModel):
    ...
    issues_type: Optional[str] = None    # ConcatWithoutMode
...
# ---------------------------------------------------------------------------
# 1) Capture ONE ABAP statement that starts with CONCATENATE and ends at the period.
STMT_RE = re.compile(r"(?is)\bCONCATENATE\b[^.]*\.", re.DOTALL)
...
def scan_unit(unit: Unit) -> Unit:
    ...
    for m in STMT_RE.finditer(src):
        ...
        if not has_mode:
            ...
            # Snippet: full line containing the CONCATENATE statement
            ...
            msg = "CONCATENATE without MODE. Add IN CHARACTER MODE (text) or IN BYTE MODE (xstring)."
            sug = (
                "CONCATENATE <a> <b> INTO lv_text IN CHARACTER MODE.\n"
                "OR\n"
                "CONCATENATE <x1> <x2> INTO lv_x IN BYTE MODE."
            )
...
@app.get("/health")
async def health():
    return {"ok": True, "rule": 101, "version": "2.0"}
